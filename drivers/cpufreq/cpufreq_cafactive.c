...
static void cpufreq_cafactive_timer_resched(
 ...
 * function.
 */
static void cpufreq_cafactive_timer_start(int cpu)
{
...
static unsigned int choose_freq(
	struct cpufreq_cafactive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_cafactive_timer_resched(pcpu);
...
static void cpufreq_cafactive_idle_start(void)
{
	struct cpufreq_cafactive_cpuinfo *pcpu =
...
static void cpufreq_cafactive_boost(void)
{
	int i;
	int anyboost = 0;
...
static struct attribute *cafactive_attributes[] = {
...
static struct attribute_group cafactive_attr_group = {
	.attrs = cafactive_attributes,
	.name = "cafactive",
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_cafactive_timer_start(j);
